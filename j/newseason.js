...
var ypObj = {
  getHasPurchase: function() {
    ...
    //   success: function(data) {
        ...
        if (data.code == 200) {
          // 首轮尝鲜礼包
          if (data.team_dj && data.team_dj.length > 0) {
            self.hasPurchase(__newseason.ns_03[0],data.team_dj);
          }
          if (data.team_yc && data.team_yc.length > 0) {
            self.hasPurchase(__newseason.ns_03[1],data.team_yc);
          }
          if (data.team_yj && data.team_yj.length > 0) {
            self.hasPurchase(__newseason.ns_03[2],data.team_yj);
          }
          if (data.team_xj && data.team_xj.length > 0) {
            self.hasPurchase(__newseason.ns_03[3],data.team_xj);
          }
        }
        self.render();
    //   }
